...
2. Purpose
   - Ensure every action is driven by clear, documented project goals.

3. Adaptability
   - Respond to changing requirements and feedback with flexible, iterative adjustments.

4. Rigor
   - Uphold high standards for quality, testing, and security in every task.
...
Symbolic Reasoning Explained

- Definition
  - Symbolic reasoning involves representing problems with symbols and rules to perform logical deduction. It allows an agent to understand code structure, identify patterns, and make decisions based on logical inference rather than just statistical analysis.
- Example
  - When refactoring, an agent should use symbolic reasoning to identify functionally identical code blocks—even if variable names differ—and propose a single, parameterized function to replace them, thus reducing redundancy.
...
3. Suggest vs. Apply
   - Clearly indicate whether AI should propose ("Suggestion:"), directly implement changes ("Applying fix:"), or ask for clarification when requirements are ambiguous ("Query:").
...
6. File Naming
   - Use descriptive, permanent, and standardized naming conventions (e.g., `[feature]/[component-name].tsx`, `[hook-name].hook.ts`).
...
3. Research Tools
   - Use advanced agentic tools to resolve complex issues efficiently:
     - **Perplexity:** For conceptual research and exploring technical solutions.
     - **AIDER.chat:** For interactive, code-centric problem-solving and debugging.
     - **Firecrawl:** For web data extraction and analysis.
...
1. Git Hygiene
   - Commit frequently with clear and descriptive messages (e.g., `feat(api): Add user authentication endpoint`).
...
1. Reflective Documentation
   - Keep comprehensive, accurate, and logically structured documentation updated through continuous, automated review.
